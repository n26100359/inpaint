...
#     cgan.train(xtrain, ytrain, batch_size=200, epochs=40000, train_D_iter=5, save_model_iter=200)
    xtrain = np.load('./xtrain32.npy',allow_pickle=True)
    ytrain = np.load('./ytrain32.npy',allow_pickle=True)
    xtest = np.load('./xtest32.npy',allow_pickle=True)
    ytest = np.load('./ytest32.npy',allow_pickle=True)
    print(np.shape(xtrain), np.shape(xtest))
    cgan = CGANv2((32,32,3), lr=0.001)
    cgan.train(xtrain, ytrain,
               ...
               save_model_iter=500,
               use_fake2=True,
               save_path='./models/GAN_art32_fine_tune/')
